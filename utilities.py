...
def produceRandomlyDeformedImage(image, label, dmap, numcontrolpoints, stdDef, Dmap_bool):
    sitkImage=sitk.GetImageFromArray(image, isVector=False)
    sitklabel=sitk.GetImageFromArray(label, isVector=False)
    if Dmap_bool:
        sitkDmap=sitk.GetImageFromArray(dmap, isVector=False)
    ...
    if Dmap_bool:
        resampler.SetDefaultPixelValue(np.mean(np.asarray(label,dtype=float)))
        outdmapsitk = resampler.Execute(sitkDmap)

    outimg = sitk.GetArrayFromImage(outimgsitk)
    outimg = outimg.astype(dtype=np.float32)

    outlbl = sitk.GetArrayFromImage(outlabsitk)
    outlbl = (outlbl).astype(dtype=np.float32)      #>0.5

    if Dmap_bool:
        outdmap = sitk.GetArrayFromImage(outdmapsitk)
        outdmap = (outdmap).astype(dtype=np.float32)
    else: outdmap=None

    return outimg,outlbl,outdmap
